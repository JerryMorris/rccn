 ...
 * <p>
 * rcc, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = rcc.getIntProperty("rcc.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for rcc monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.rcc ? holdingId : 0);
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.rcc || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            rcc.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (rcc.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case rcc:
                                        processrccEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws  rccException                Unable to create transaction
     */
    private static void processrccEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
                                            throws rccException {
        FundingMonitor monitor = monitoredAccount.monitor;
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = rcc.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(rcc.getBlockchain().getLastBlockTimestamp());
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                monitoredAccount.height = rcc.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("rcc funding transaction %s for %f rcc submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_rcc,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the rcc balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.rcc && balance < maccount.threshold &&
