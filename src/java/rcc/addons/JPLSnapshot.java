...
public final class JPLSnapshot implements AddOn {
     ...
     * requirement to existing rcc holders.</p>
     *
     * <p>This utility takes a snapshot of account balances and public keys on the rcc blockchain as of the specified height,
     ...
     * <li>height - the rcc blockchain height at which to take the snapshot</li>
     * </ul>
     *
     * <p>Response</p>
     ...
     * for a total of 900M to new accounts, resulting in 100M automatically allocated to existing rcc holders:
     * <pre>
     * {
     *     "balances": {
    ...
    public static class JPLSnapshotAPI extends APIServlet.APIRequestHandler {
        ...
        protected JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws rccException {
            ...
            rcc.getBlockchainProcessor().addListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
            rcc.getBlockchainProcessor().scan(height - 1, false);
            rcc.getBlockchainProcessor().removeListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
    ...
    private static class JPLSnapshotListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            if (block.getHeight() == height) {
                ...
                SortedMap<String, Long> snapshotrccBalances = snapshotrccBalances();
                BigInteger snapshotTotal = BigInteger.valueOf(snapshotrccBalances.values().stream().mapToLong(Long::longValue).sum());
                ...
                if (inputBalances != null) {
                    ...
                    if (!inputTotal.equals(BigInteger.ZERO)) {
                        snapshotrccBalances.entrySet().forEach(entry -> {
                    ...
                    inputBalances.entrySet().forEach(entry -> {
                        ...
                        snapshotrccBalances.merge(account, inputBalance, (a, b) -> a + b);
                    });
                }
                snapshot.put("balances", snapshotrccBalances);
            }
        }
        ...
        private SortedMap<String, Long> snapshotrccBalances() {
