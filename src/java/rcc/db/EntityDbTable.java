...
public abstract class EntityDbTable<T> extends TrimmableDbTable<T> {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && rcc.getBlockchainProcessor().isScanning() ?
                    Math.max(rcc.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return rcc.getBlockchain().getHeight() <= height && ! (isPersistent() && rcc.getBlockchainProcessor().isScanning());
    }
