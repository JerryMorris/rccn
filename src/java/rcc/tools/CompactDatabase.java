 ...
 *
 *   java -cp "classes:lib/*:conf" rcc.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Drcc.runtime.mode=desktop rcc.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize rcc properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "rcc.testDb" : "rcc.db";
        String dbType = rcc.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = rcc.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = rcc.getDbDir(rcc.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = rcc.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        String dbUsername = rcc.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = rcc.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "rcc.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "rcc.mv.db");
            if (!dbFile.exists()) {
