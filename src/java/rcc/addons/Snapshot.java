...
public class Snapshot implements AddOn {
    ...
    private static final int snapshotHeight = rcc.getIntProperty("rcc.snapshotHeight", Integer.MAX_VALUE);
    private static final boolean snapshotForTestnet = rcc.getBooleanProperty("rcc.snapshotForTestnet", true);

    private static final Set<String> scammerAccounts = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
            Long.toUnsignedString(Convert.parseAccountId("rcc-TN8U-RBVE-GBJ3-7DEBN")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-V79Z-RQ5X-XXJR-H8P87")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-BH28-PKY6-LES8-29DBN")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-ZKV3-J2WN-T6VM-B28DA")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-ZPRA-ZDUQ-SYEL-7AAJM")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-UMZH-XLBB-BGSY-7WESP")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-5CFL-QTTH-D6K2-AC4TF")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-B2KJ-DAAF-884G-GENQ8")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-L8JG-U967-NUNS-BA4RK")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("rcc-LSMJ-YCH7-QESX-AH42N")), // compromised
            Long.toUnsignedString(Convert.parseAccountId("rcc-E8JD-FHKJ-CQ9H-5KGMQ"))  // compromised
    )));

    private static final String wrongBountyAccount = Long.toUnsignedString(Convert.parseAccountId("rcc-XTJE-PLDX-EZ6E-6FQX6"));

    private static final String AEUR_ACCOUNT = "rcc-NJ92-R5GB-HQB4-6NW7T";
            ...
            private Map<String, Long> exportIgnisBalances() {
                        ...
                        while (rs.next()) {
                            ...
                            if (snapshotForTestnet) {
                                eurSnapshotMap.put(account, Convert.longValueExact(
                                        ...
                                        .divide(BigInteger.valueOf(Constants.ONE_rcc))));
                            } else {
                ...
                if (snapshotForTestnet && !developerPublicKeys.isEmpty()) {
                    ...
                    developerPublicKeys.forEach(publicKey -> {
                        ...
                        eurSnapshotMap.put(account, (developerBalance / Constants.ONE_rcc) * 10000);
                    });
                }
