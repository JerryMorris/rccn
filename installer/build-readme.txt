Building the rcc installer

Note: all pathes are relative to the git repository folder (rcc-private)
...
Test X11 installer: java -jar rcc-client-<version>.jar
Test console installer: java -jar rcc-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your rcc-private git repository preserving the folder structure
...
Test the installer using Java: java -jar rcc-client-<version>.jar
Test the windows executable: rcc-client-<version>.exe
...
rcc.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
