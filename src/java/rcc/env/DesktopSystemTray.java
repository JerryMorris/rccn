...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/www/img/rcc-icon-32x32.png", "tray icon");
        ...
        if (!rcc.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("rccdesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("rccdesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("rccdesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("rccdesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", rcc.APPLICATION);
        addDataRow(statusPanel, "Version", rcc.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(rcc.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(rcc.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", rcc.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "rcc Server Status");
