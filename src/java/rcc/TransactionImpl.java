...
import rcc.rccException.NotValidException;
...
final class TransactionImpl implements Transaction {
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws NotValidException {
        ...
        if (builder.isGenesisBlock) {
            ...
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : rcc.getBlockchain().getHeight());
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_rcc));
                    buffer.putInt((int) (feeNQT / Constants.ONE_rcc));
                    if (referencedTransactionFullHash != null) {
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || rcc.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    public void validate() throws rccException.ValidationException {
        ...
        if (!validatingAtFinish) {
            ...
            if (feeNQT < minimumFeeNQT) {
                throw new rccException.NotCurrentlyValidException(String.format("Transaction fee %f rcc less than minimum fee %f rcc at height %d",
                        ((double) feeNQT) / Constants.ONE_rcc, ((double) minimumFeeNQT) / Constants.ONE_rcc, blockchainHeight));
            }
            if (blockchainHeight > Constants.FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new rccException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_rcc);
        }
        return totalFee;
    }
