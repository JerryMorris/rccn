...
import rcc.ShufflingTransaction;
import rcc.Tester;
import rcc.Transaction;
import rcc.crypto.AnonymouslyEncryptedData;
...
import static rcc.http.shuffling.ShufflingUtil.startShuffler;
import static rcc.http.shuffling.ShufflingUtil.startShufflerBuilder;
import static rcc.http.shuffling.ShufflingUtil.stopShuffler;
...
public class TestAutomatedShuffling extends BlockchainTest {
    ...
    public void successfulShuffling() {
        ...
        Assert.assertEquals(-(defaultShufflingAmount + 11 * Constants.ONE_rcc), ALICE.getUnconfirmedBalanceDiff());
    ...
    public void duplicateProcessDataBob() {
        ...
        Assert.assertEquals(54 * Constants.ONE_rcc + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(54 * Constants.ONE_rcc + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedRecipientKeysDave() {
        ...
        Assert.assertEquals(-65 * Constants.ONE_rcc, ALICE.getBalanceDiff() + BOB.getBalanceDiff() + CHUCK.getBalanceDiff());
    ...
    public void duplicateRecipientsBobChuck() {
        ...
        startShuffler(BOB, BOB_RECIPIENT, shufflingFullHash);
        generateBlock();
        JSONObject startShufflerResponse = startShufflerBuilder(CHUCK, BOB_RECIPIENT, shufflingFullHash).invoke();

        assertEquals(
                "Incorrect \"recipientPublicKey\" Another shuffler with the same recipient account already running",
                startShufflerResponse.get("errorDescription"));
    }
    ...
    public void maxShufflingSize() {
        ...
        for (int i = 0; i < n + 5; i++) {
            generateBlock();
            Block block = rcc.getBlockchain().getLastBlock();
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_rcc) + 100 * Constants.ONE_rcc, participants[n - 1].getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_rcc) + 100 * Constants.ONE_rcc, participants[n - 1].getUnconfirmedBalanceDiff());
