...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws rccException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new rccException.NotValidException("Invalid block signature");
            }
            return block;
        } catch (rccException.NotValidException|RuntimeException e) {
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_rcc));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_rcc));
            } else {
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalancercc();
    ...
    void apply() {
        ...
        if (this.height > Math.max(Constants.SHUFFLING_BLOCK, 3)) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f rcc to forger at height %d", ((double)backFees[i])/Constants.ONE_rcc, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f rcc at height %d", ((double)totalBackFees)/Constants.ONE_rcc, this.height);
        }
    ...
    private void calculateBaseTarget(BlockImpl previousBlock) {
        ...
        if (previousBlock.getHeight() < Math.max(Constants.SHUFFLING_BLOCK, 3)) {
