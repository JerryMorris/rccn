...
import rcc.AccountLedger.LedgerEntry;
import rcc.Block;
 ...
 * has not been received within rcc.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by rcc.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = rcc.getIntProperty("rcc.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(rcc.getIntProperty("rcc.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = rcc.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = rcc.getTransactionProcessor();
    ...
    /** rcc event listeners */
    private final List<rccEventListener> rccEventListeners = new ArrayList<>();
     ...
     *
     * rcc event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of rcc event registrations
     ...
     *
     * @param   eventRegistrations      rcc event registrations
     * @throws  EventListenerException  Invalid rcc event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<rccEventListener> it = rccEventListeners.iterator();
                while (it.hasNext()) {
                    rccEventListener listener = it.next();
                ...
                if (addListener) {
                    rccEventListener listener = new rccEventListener(event);
                    listener.addListener();
                    rccEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            rccEventListeners.forEach(rccEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * rcc event listener
     */
    private class rccEventListener {

        /** Event handler */
        private final rccEventHandler eventHandler;

        /**
         * Create the rcc event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public rccEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the rcc listener for this event
         */
        ...
        /**
         * Remove the rcc listener for this event
         */
        ...
        /**
         * Return the hash code for this rcc event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two rcc events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof rccEventListener) &&
                    eventHandler.equals(((rccEventListener)obj).eventHandler));
        }

        /**
         * rcc listener event handler
         */
        private abstract class rccEventHandler {
            ...
            /**
             * Create the rcc event handler
             *
             * @param   eventRegistration   Event registration
             */
            public rccEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof rccEventHandler) &&
                                        owner == ((rccEventHandler)obj).owner &&
                                        accountId == ((rccEventHandler)obj).accountId &&
                                        event == ((rccEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends rccEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends rccEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends rccEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends rccEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           rcc listener event
        ...
        /**
         * Return the rcc listener event
         *
         * @return                  rcc listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
