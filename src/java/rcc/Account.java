...
import rcc.AccountLedger.LedgerHolding;
...
import rcc.db.VersionedEntityDbTable;
import rcc.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, rcc.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    private static final VersionedPersistentDbTable<PublicKey> publicKeyTable = new VersionedPersistentDbTable<PublicKey>("public_key", publicKeyDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height > rcc.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + rcc.getBlockchain().getHeight());
            }
        }
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        @Override
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < rcc.getBlockchainProcessor().getMinRollbackHeight()) {
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = rcc.getBooleanProperty("rcc.enablePublicKeyCache") ?
    ...
    static {
        ...
        rcc.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            rcc.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalancercc() {
        return getEffectiveBalancercc(rcc.getBlockchain().getHeight());
    }

    public long getEffectiveBalancercc(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_rcc;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : rcc.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_rcc;
        }
        ...
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_rcc;
        } finally {
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, rcc.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < rcc.getBlockchainProcessor().getMinRollbackHeight()
                    || height > rcc.getBlockchain().getHeight()) {
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = rcc.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= rcc.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.rcc_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.rcc_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_rcc_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_rcc_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
