...
import static rcc.http.JSONResponses.INCORRECT_ALIAS;
import static rcc.http.JSONResponses.INCORRECT_ARBITRARY_MESSAGE;
import static rcc.http.JSONResponses.INCORRECT_DATA_TOO_LONG;
import static rcc.http.JSONResponses.INCORRECT_DATA_ZERO_LENGTH;
import static rcc.http.JSONResponses.INCORRECT_HEIGHT;
import static rcc.http.JSONResponses.INCORRECT_MESSAGE_TO_ENCRYPT;
import static rcc.http.JSONResponses.INCORRECT_PURCHASE;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_CHANNEL;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_DESCRIPTION;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_FILE;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_FILENAME;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_NAME;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_TAGS;
import static rcc.http.JSONResponses.INCORRECT_TAGGED_DATA_TYPE;
import static rcc.http.JSONResponses.MISSING_ACCOUNT;
import static rcc.http.JSONResponses.MISSING_ALIAS_OR_ALIAS_NAME;
import static rcc.http.JSONResponses.MISSING_NAME;
import static rcc.http.JSONResponses.MISSING_PROPERTY;
import static rcc.http.JSONResponses.MISSING_RECIPIENT_PUBLIC_KEY;
import static rcc.http.JSONResponses.MISSING_SECRET_PHRASE;
import static rcc.http.JSONResponses.MISSING_TRANSACTION_BYTES_OR_JSON;
import static rcc.http.JSONResponses.UNKNOWN_ACCOUNT;
import static rcc.http.JSONResponses.UNKNOWN_ALIAS;
import static rcc.http.JSONResponses.UNKNOWN_ASSET;
import static rcc.http.JSONResponses.UNKNOWN_CURRENCY;
import static rcc.http.JSONResponses.UNKNOWN_GOODS;
import static rcc.http.JSONResponses.UNKNOWN_OFFER;
import static rcc.http.JSONResponses.UNKNOWN_POLL;
import static rcc.http.JSONResponses.UNKNOWN_SHUFFLING;
import static rcc.http.JSONResponses.either;
import static rcc.http.JSONResponses.incorrect;
import static rcc.http.JSONResponses.missing;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, rcc.getBlockchain().getHeight(), false);
    }
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.rcc);
        if (holdingType == HoldingType.rcc && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is rcc"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return rcc.newTransactionBuilder(json);
            } catch (rccException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return rcc.newTransactionBuilder(bytes, prunableAttachments);
            } catch (rccException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, rccException.NotValidException {
