...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedrccAccounts") && NRS.getStrItem("savedrccAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedrccAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var rccAddress = new rccAddress();
                if (rccAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedrccAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedrccAccounts');
		} else {
			NRS.setStrItem("savedrccAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedrccAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedrccAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedrccAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedrccAccounts") && NRS.getStrItem("savedrccAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedrccAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your rcc passphrase.";
			html += "</p>";
			...
			html += "you can loose your rcc! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of rcc now or in the future.";
			html += "</p>";
			html += "</div>";
