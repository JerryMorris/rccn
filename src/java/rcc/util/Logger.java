...
/**
 * Handle logging for the rcc node server
 */
public final class Logger {
     ...
     * The rcc/conf/logging-default.properties and rcc/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "rcc.util.rccLogManager");
        if (!(LogManager.getLogManager() instanceof rccLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("rcc.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                rcc.loadProperties(loggingProperties, "logging-default.properties", true);
                rcc.loadProperties(loggingProperties, "logging.properties", false);
                rcc.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(rcc.rcc.class);
        enableStackTraces = rcc.getBooleanProperty("rcc.enableStackTraces");
        enableLogTraceback = rcc.getBooleanProperty("rcc.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof rccLogManager) {
            ((rccLogManager) LogManager.getLogManager()).rccShutdown();
        }
    }
