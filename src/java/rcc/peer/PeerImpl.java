...
import rcc.util.CountingInputReader;
import rcc.util.CountingInputStream;
import rcc.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (rcc.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < rcc.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = rcc.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_rcc) / Constants.MAX_BALANCE_rcc);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof rccException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = rcc.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/rcc"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new rccException.rccIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/rcc");
                if (communicationLoggingMask != 0)
        ...
        } catch (rccException.rccIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = rcc.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_rcc * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
