...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (rcc.getBooleanProperty("rcc.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = rcc.getIntProperty("rcc.apiProxyBlacklistingPeriod");
    static final String forcedServerURL = rcc.getStringProperty("rcc.forceAPIProxyServerURL", "");
    public static final int PEER_CONNECTIONS_RETRIES = 3;
    private final List<String> bootstrapNodes = rcc.getStringListProperty(
            Constants.isTestnet ? "rcc.testnetProxyBootstrapNodes" : "rcc.proxyBootstrapNodes");
    ...
    private JSONObject executeRequest(Peer peer, String requestType, String query) {
        ...
        try {
            uri.append("/rcc?requestType=").append(requestType);
            if (query != null) {
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && rcc.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        int unblacklistTime = rcc.getEpochTime() + blacklistingPeriod;
        blacklistedPeers.put(host, unblacklistTime);
