...
jar/exe/dmg package, or run "java -jar rcc-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/rcc/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the rcc server.


Using the rcc-client.zip package:

Unpack the rcc-client.zip package and open a shell in the resulting rcc
...
the rcc_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.rcc for
configuration files, logs, and the rcc_db database. Unlike run.sh, start.sh
...
message "rcc server 1.x.x started successfully" in the console log. If run in
...
where the rcc UI should now be available.
...
to the rcc installation directory, as the use of special characters may result
...
conf/rcc-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
rcc-default.properties directly, but create a new conf/rcc.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from rcc-default.properties, the
settings in rcc.properties override those in rcc-default.properties. This way,
when upgrading the software, you can safely overwrite rcc-default.properties
with the updated file from the new package, while your customizations remain
safe in the rcc.properties file.


How to contribute?

There are many ways to contribute to rcc. Here are some examples:
...
The rcc software is a client-server application. It consists of a java server
...
requests which other rcc client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
rcc_db directory. When upgrading, you should not delete the old rcc_db
...
rcc_db, except that it will take some extra time to download the blockchain
from scratch.

The default rcc client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file rcc.log, which gets
overwritten at restart. In case of an error, the rcc.log file may contain
...
rcc.jar file using the enclosed jar.sh script, and then rcc.jar should be
