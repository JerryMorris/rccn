...
class StringLengthRw implements LengthRw {
    ...
    @Override
    public int readFromBuffer(ByteBuffer buffer) throws rccException.NotValidException {
        int result = lengthRw.readFromBuffer(buffer);

        if (!validate(result)) {
            throw new rccException.NotValidException("Read invalid length " + result);
        }
