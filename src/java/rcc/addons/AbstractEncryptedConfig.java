...
public abstract class AbstractEncryptedConfig implements AddOn {
    ...
    protected Path getDefaultPath() {
        Path path = Paths.get(rcc.getStringProperty("rcc.addons.EncryptedConfig.path", "conf/processes/"), getDefaultFilename());
        if (!path.isAbsolute()) {
            path = Paths.get(rcc.getUserHomeDir()).resolve(path).toAbsolutePath();
        }
        return path;
    }
