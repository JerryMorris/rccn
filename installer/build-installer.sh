...
CONFDIR=$CLASSDIR/rcc/installer/resources
JARFILE=$CLASSDIR/rcc-panels.jar

# build custom panels
rm -rf $CLASSDIR
mkdir -p $CLASSDIR
${JAVAC} -cp "../installer/lib/*" -d $CLASSDIR -sourcepath ../installer/panels/src ../installer/panels/src/rcc/installer/*.java

mkdir -p $CONFDIR
cp ../conf/examples/*.properties $CONFDIR
(cd $CONFDIR; ls *.properties > settings.txt)

${JAR} cf0 $JARFILE -C $CLASSDIR rcc
