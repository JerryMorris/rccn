...
public final class DebugTrace {
    ...
    static final String QUOTE = rcc.getStringProperty("rcc.debugTraceQuote", "\"");
    static final String SEPARATOR = rcc.getStringProperty("rcc.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = rcc.getBooleanProperty("rcc.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = rcc.getStringListProperty("rcc.debugTraceAccounts");
        String logName = rcc.getStringProperty("rcc.debugTraceLog");
        ...
        rcc.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        rcc.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        rcc.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        rcc.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = rcc.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(rcc.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(rcc.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalancercc()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalancercc()));
