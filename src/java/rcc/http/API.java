...
import static rcc.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static rcc.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static rcc.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static rcc.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = rcc.getStringProperty("rcc.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = rcc.getIntProperty("rcc.maxAPIRecords");
    static final boolean enableAPIUPnP = rcc.getBooleanProperty("rcc.enableAPIUPnP");
    public static final int apiServerIdleTimeout = rcc.getIntProperty("rcc.apiServerIdleTimeout");
    public static final boolean apiServerCORS = rcc.getBooleanProperty("rcc.apiServerCORS");
    private static final String forwardedForHeader = rcc.getStringProperty("rcc.forwardedForHeader");
    ...
    static {
        List<String> allowedBotHostsList = rcc.getStringListProperty("rcc.allowedBotHosts");
        ...
        boolean enableAPIServer = rcc.getBooleanProperty("rcc.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : rcc.getIntProperty("rcc.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : rcc.getIntProperty("rcc.apiServerSSLPort");
            final String host = rcc.getStringProperty("rcc.apiServerHost");
            disableAdminPassword = rcc.getBooleanProperty("rcc.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = rcc.getBooleanProperty("rcc.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(rcc.getUserHomeDir()).resolve(Paths.get(rcc.getStringProperty("rcc.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(rcc.getStringProperty("rcc.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(rcc.getStringProperty("rcc.keyStoreType"));
                List<String> ciphers = rcc.getStringListProperty("rcc.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = rcc.getStringProperty("rcc.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{rcc.getStringProperty("rcc.apiWelcomeFile")});
            }

            String javadocResourceBase = rcc.getStringProperty("rcc.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/rcc");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(rcc.getIntProperty("rcc.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/rcc-proxy");
            ...
            if (!rcc.getBooleanProperty("rcc.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/rcc", "/rcc-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(rcc.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (rcc.getBooleanProperty("rcc.apiFrameOptionsSameOrigin")) {
                FilterHolder filterHolder = apiHandler.addFilter(XFrameOptionsFilter.class, "/*", null);
                filterHolder.setAsyncSupported(true);
            }
            disableHttpMethods(apiHandler);

            String customAPISetupImplClassName = Convert.emptyToNull(rcc.getStringProperty("rcc.apiCustomSetupImpl"));
    ...
    public static void init() {
        if (!isInitialized) {
            isInitialized = true;
            List<String> disabled = new ArrayList<>(rcc.getStringListProperty("rcc.disabledAPIs"));
            ...
            disabled = rcc.getStringListProperty("rcc.disabledAPITags");
