...
import static rcc.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static rcc.http.JSONResponses.INCORRECT_DEADLINE;
import static rcc.http.JSONResponses.INCORRECT_EC_BLOCK;
import static rcc.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static rcc.http.JSONResponses.INCORRECT_WHITELIST;
import static rcc.http.JSONResponses.MISSING_DEADLINE;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                rcc.getBlockchain().getHeight() + 1,
                rcc.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws rccException {
        ...
        if (ecBlockId != 0 && ecBlockId != rcc.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = rcc.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = rcc.newTransactionBuilder(publicKey, amountNQT, feeNQT,
        ...
        } catch (rccException.NotYetEnabledException e) {
